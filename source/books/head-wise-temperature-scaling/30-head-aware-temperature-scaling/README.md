...
Note that our correlated Gaussian query–key setup corresponds to the Gaussian logits assumption in [Anson et al., 2025] as $d \to \infty$, and generalizes the independent query–key model that yields zero-mean logits [Barbero et al., 2025], thereby capturing effects beyond the initialization regime.
...
![Polynomial target ESS scaling under the NoPE Gaussian toy model.](toy_adascale__toy_ESS_poly.png)

**Figure 10.** Polynomial target ESS scaling under the NoPE Gaussian toy model.
...
![Constant target ESS under the NoPE Gaussian toy model.](toy_adascale__toy_ESS_const.png)

**Figure 11.** Constant target ESS under the NoPE Gaussian toy model.
...
![Illustration of $\mu_i$ under RoPE with $L=4096$, $b=10000$, and $\rho=1$. By Eq. (3), $\mu_i = \rho \int_{0}^{1} \cos\left((i-L) b^{-x}\right) \mathop{}\!\mathrm{d} x$ for $i \in \left\{1, \dots, L\right\}$.](rope_mu_i__rope_mu_i_profile.png)

**Figure 12.** Illustration of $\mu_i$ under RoPE with $L=4096$, $b=10000$, and $\rho=1$. By Eq. (3), $\mu_i = \rho \int_{0}^{1} \cos\left((i-L) b^{-x}\right) \mathop{}\!\mathrm{d} x$ for $i \in \left\{1, \dots, L\right\}$.
